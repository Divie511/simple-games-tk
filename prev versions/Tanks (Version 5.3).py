...
class beam:
    def __init__(self):
        if warriorA == w1:
            self.rectangle = canvas73.create_rectangle(75,125,100,135, fill = "red")
            self.x = 5
        else:
            self.rectangle = canvas73.create_rectangle(295,125,320,135, fill = "red")
            self.x = -5
        self.a = 0
        self.move()
    def move(self):
        if self.a == 50:
            canvas73.delete(self.rectangle)
            ChangeGameState()
            return
        self.a += 1
        canvas73.move(self.rectangle, self.x, 0)
