...
def on_leave(event=None,*args):
    ...
    a2.config(fg='#fff')
    a3.config(fg='#fff')
def set_tank_colour1(col):
    global w1
    w1 = Tank(playerdata.userlist.get(u1),col)
def  set_tank_colour2(col):
    global w2
    w2 = Tank(playerdata.userlist.get(u2),col)
    colourCanvas.destroy()
    LoadGame(u1,u2)
def tank_colour():
    global colourCanvas
    introFrame.destroy() # introFrame is where you recieve the introduction message. Now that the game has begun, we delete it.
    colourCanvas=Canvas(game_window,height=250,width=400,bg='#000')
    colourCanvas.pack()
    blue1=Button(colourCanvas,text='w1',bg='blue',command=lambda: set_tank_colour1('blue')).grid(row=1,column=1)
    blue2=Button(colourCanvas,text='w2',bg='blue',command=lambda: set_tank_colour2('blue')).grid(row=1,column=2)
    green1=Button(colourCanvas,text='w1',bg='green',command=lambda: set_tank_colour1('green')).grid(row=2,column=1)
    green2=Button(colourCanvas,text='w2',bg='green',command=lambda: set_tank_colour2('green')).grid(row=2,column=2)
    red1=Button(colourCanvas,text='w1',bg='red',command=lambda: set_tank_colour1('red')).grid(row=3,column=1)
    red2=Button(colourCanvas,text='w2',bg='red',command=lambda: set_tank_colour2('red')).grid(row=3,column=2)
    orange1=Button(colourCanvas,text='w1',bg='orange',command=lambda: set_tank_colour1('orange')).grid(row=4,column=1)
    orange2=Button(colourCanvas,text='w2',bg='orange',command=lambda: set_tank_colour2('orange')).grid(row=4,column=2)
...
class Tank:
    ...
    def __init__(self,player,tank_col):
        try:
            ...
            quit()
        self.colourx=tank_col
        self.coloury=tank_col
    ...
    def tank(self):
        if w1==self:
            self.base=canvas73.create_rectangle(0,150,75,225,fill=self.colourx,outline=self.colourx)
            self.top=canvas73.create_rectangle(30,100,50,150,fill=self.colourx,outline=self.colourx)
            self.turret=canvas73.create_rectangle(50,125,75,135,fill=self.colourx,outline=self.colourx)
            self.front=canvas73.create_polygon([75,150,75,225,100,225,75,150],fill=self.colourx)
            self.behind=canvas73.create_polygon([0,150,0,70,75,90,20,90,20,150,0,150],fill=self.colourx)
    
        else:
            self.base=canvas73.create_rectangle(325,150,400,225,fill=self.coloury,outline=self.coloury)
            self.top=canvas73.create_rectangle(350,100,370,150,fill=self.coloury,outline=self.coloury)
            self.turret=canvas73.create_rectangle(325,125,350,135,fill=self.coloury,outline=self.coloury)
            self.front=canvas73.create_polygon([325,150,325,225,300,225,325,150],fill=self.coloury)
            self.behind=canvas73.create_polygon([400,150,400,70,325,90,380,90,380,150,400,150],fill=self.coloury)
    ...
    def shield(self):
        ...
        if self==w1:
            player_shield=canvas73.create_polygon([100,125,120,125,150,150,120,170,150,200,120,225,100,225,100,125],fill=self.colourx)
            canvas73.after(1000,self.delete_shield)   
        else:
            player_shield=canvas73.create_polygon([300,125,280,125,250,150,280,170,250,200,280,225,300,225,300,215],fill=self.coloury)
            canvas73.after(1000,self.delete_shield)
