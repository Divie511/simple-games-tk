...
def on_leave(event=None,*args):
    global messagebox
    messagebox.configure(text = f'It\'s {warriorA}\'s turn to attack.', fg = "#fff")
    a1.config(fg='#fff')
    a2.config(fg='#fff')
    a3.config(fg='#fff')
...
def SetGame(): # this function will make display changes acc to GameState
    ...
    if GameState == 1: # ATTACK mode
        ...
        a1.config(bg = 'grey', command = DoNothing, cursor = 'arrow') # disabling attack buttons
        a2.config(bg = 'grey', command = DoNothing, cursor = 'arrow') # ChangeGameState will switch the game back to attack mode
        a3.config(bg = 'grey', command = DoNothing, cursor = 'arrow')
    ...
    if GameState == 10:
        messagebox.config(text = f'{warriorA} uses {HT}.', fg = "#fff")
        a1.config(fg='#fff')
        a2.config(fg='#fff')
        a3.config(fg='#fff')
        if hit != -1: beam()
        ...
        elif dmg == 0: commentline = f'Attack successfully blocked by {warriorB}!'
    ...
    if GameState == 100: # If the one of the players dies
        ...
        canvas73.delete(warriorB.front)
        canvas73.delete(warriorB.behind)
        winScreen()
...
class Warrior:
    ...
    def tank(self):
        if w1==self:
            ...
            self.turret=canvas73.create_rectangle(325,125,355,135,fill="#ff0000",outline="#ff0000")
