...
def Login(*args):
    ...
    reload(playerdata) # loads modified playerdata file with new users registered
...
class Tank:
    ...
    def blow(self):
        x = -3 if self == w1 else 3
        if self.health < self.maxHealth/2: canvas73.move(self.behind_upper,x,-2)
        if self.health < self.maxHealth/5: canvas73.move(self.behind,x,-2)
    
        
    def tank(self,master,colour):
        self.colour = colour
        if w1==self:
            self.base=master.create_rectangle(30,200,55,224,fill=self.colour,outline=self.colour)
            self.top=master.create_rectangle(45,170,50,200,fill=self.colour,outline=self.colour)
            if self.level>4:
                self.turret=master.create_polygon([50,180,52,180,52,178,54,178,54,180,56,180,56,178,58,178,58,180,60,180,60,178,62,178,62,180,64,180,64,178,66,178,66,180,68,180,68,178,70,178,70,180,75,180,75,185,70,185,70,187,68,187,68,185,66,185,66,187,65,187,64,185,62,185,62,187,60,187,60,185,50,185,50,180,],fill=self.colour)
            else:
                self.turret=master.create_rectangle(50,180,75,185,fill=self.colour,outline=self.colour)
            self.turret2=master.create_rectangle(50,170,75,175,fill=self.colour,outline=self.colour)
            ...
            x = int(master['width'])
            self.base=master.create_rectangle(x-55,200,x-30,224,fill=self.colour,outline=self.colour)
            self.top=master.create_rectangle(x-50,200,x-45,170,fill=self.colour,outline=self.colour)
            if self.level>4:
                self.turret=master.create_polygon([x-50,180,x-52,180,x-52,178,x-54,178,x-54,180,x-56,180,x-56,178,x-58,178,x-58,180,x-60,180,x-60,178,x-62,178,x-62,180,x-64,180,x-64,178,x-66,178,x-66,180,x-68,180,x-68,178,x-70,178,x-70,180,x-75,180,x-75,185,x-70,185,x-70,187,x-68,187,x-68,185,x-66,185,x-66,187,x-64,187,x-64,185,x-62,185,x-62,187,x-60,187,x-60,185,x-50,185,x-50,180,],fill=self.colour)
            else:
                self.turret=master.create_rectangle(x-75,180,x-50,185,fill=self.colour,outline=self.colour)
            self.turret2=master.create_rectangle(x-50,170,x-75,175,fill=self.colour,outline=self.colour)
    ...
    def lose(self):
        ...
        canvas73.move(self.turret2,-7*x,-5)
    ...
    def setActive(self):
        ...
        if self.health < self.maxHealth/2: canvas73.delete(self.behind_upper)
        if self.health < self.maxHealth/5: canvas73.delete(self.behind)
...
class beam:
    def __init__(self):
        if TankA == w1:
            ...
            if dmg>25:
                self.laser2=canvas73.create_rectangle(60,170,85,175,fill='#ff0000')                
            self.x = 5
        else:
            self.laser = canvas73.create_rectangle(315,180,350,185, fill = '#ff0000')
            if dmg>25:
                self.laser2=canvas73.create_rectangle(315,170,350,175,fill='#ff0000')
            self.x = -5
        self.a = 0
        self.move()
    def move(self):
        if (self.a == 45 and dmg == 0) or (self.a == 55):
            ...
            return
        if self.a < 10:
            canvas73.move(TankA.turret, -self.x/4, 0)#recoil
            if dmg>25: canvas73.move(TankA.turret2, -self.x/4, 0)
        elif self.a < 20:
            canvas73.move(TankA.turret, self.x/4, 0)
            if  dmg>25: canvas73.move(TankA.turret2, self.x/4, 0)
        self.a += 1
        canvas73.move(self.laser, self.x, 0)
        if dmg>25: canvas73.move(self.laser2,self.x,0)
...
class blast:
    def __init__(self,laser):
        ...
        if dmg>25: canvas73.delete(laser.laser2)
        if TankB.health<(TankB.maxHealth/3):
            TankB.blow()
    ...
    def move(self):
        ...
        if self.dmg > 0:
            TankB.takeDamage()
            self.dmg -= 1
        if TankB.health < TankB.maxHealth/2: TankB.blow()
        canvas73.after(10, self.move)
...
class Timer:
    ...
    def __init__(self):
        self.count = 10
        self.display = canvas73.create_text(200,239,fill="#fff",text=self.count,font=(None,16))
    def time(self):
        global GameState
        if self.count == 0: GameState = 0
        if GameState == 1:
            canvas73.itemconfig(self.display,text=self.count)
            self.count -= 1
            canvas73.after(1000,self.time)
        if GameState == 0: ChangeGameState()
    def reset(self):
        self.count = 10
        self.time()
...
def LoadObjects():
    global timer
    for i in range(40):
        canvas73.create_text(400*random(),220*random(),fill="#fff",text="*") # stars
    w1.tank(canvas73,w1.colour)
    w2.tank(canvas73,w2.colour)
    canvas73.create_rectangle(0,225,400,250,fill="#a52a2a") # ground
    #timer = Timer()
