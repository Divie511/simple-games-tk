...
def LoadGame(u1,u2):
    ...
    frame4.pack()
    a1 = Button(frame4, text = warriorA.a1, width = 18, height = 2, fg = '#fff', bg = '#000')
    a2 = Button(frame4, text = warriorA.a2, width = 18, height = 2, fg = '#fff', bg = '#000')
    a3 = Button(frame4, text = warriorA.a3, width = 18, height = 2, fg = '#fff', bg = '#000')
    a1.pack(side=LEFT)
    a3.pack(side = RIGHT)
    a2.pack()
...
def SetGame(): # this function will make display changes acc to GameState
    ...
    if GameState == 1: # ATTACK mode
        ...
        if warriorA == w1:
            ...
            l1.config(bg = '#222')
            l2.config(bg = '#000')
        a1.config(bg = '#000', command = atk1, cursor = 'hand2', text = warriorA.a1) # Enabling attack buttons
        a2.config(bg = '#000', command = atk2, cursor = 'hand2', text = warriorA.a2)
        a3.config(bg = '#000', command = atk3, cursor = 'hand2', text = warriorA.a3)
    ...
    if GameState == 100: # If the one of the players dies
        ...
        canvas73.delete(warriorB.top)
        canvas73.delete(warriorB.turret)
        canvas73.delete(warriorB.base)
        winScreen()
...
class beam:
    def __init__(self):
        if warriorA == w1:
            self.rectangle1 = canvas73.create_rectangle(75,125,100,135, fill = "yellow")
            self.rectangle2 = canvas73.create_rectangle(85,145,110,155, fill = "orange")
            self.rectangle3 = canvas73.create_rectangle(95,165,120,175, fill = "red")
            self.rectangle4 = canvas73.create_rectangle(85,185,110,195, fill = "orange")
            self.rectangle5 = canvas73.create_rectangle(75,205,100,215, fill = "yellow")
            self.x = 5
        else:
            self.rectangle1 = canvas73.create_rectangle(295,125,320,135, fill = "yellow")
            self.rectangle2 = canvas73.create_rectangle(285,145,310,155, fill = "orange")
            self.rectangle3 = canvas73.create_rectangle(275,165,300,175, fill = "red")
            self.rectangle4 = canvas73.create_rectangle(285,185,310,195, fill = "orange")
            self.rectangle5 = canvas73.create_rectangle(295,205,320,215, fill = "yellow")
            self.x = -5
        self.a = 0
        self.move()
    def move(self):
        if self.a == 50:
            canvas73.delete(self.rectangle1)
            canvas73.delete(self.rectangle2)
            canvas73.delete(self.rectangle3)
            canvas73.delete(self.rectangle4)
            canvas73.delete(self.rectangle5)
            ChangeGameState()
            return
        self.a += 1
        canvas73.move(self.rectangle1, self.x, 0)
        canvas73.move(self.rectangle2, self.x, 0)
        canvas73.move(self.rectangle3, self.x, 0)
        canvas73.move(self.rectangle4, self.x, 0)
        canvas73.move(self.rectangle5, self.x, 0)
        canvas73.after(10, self.move)

def accurate(x): # trying prob~ability to see if attack works or not
...
def atk1(*args): # These functions will be executed when the attack buttons are clicked (in GameState 1)
    global hit, HT
    HT = warriorA.a1
    c = 1 if warriorA.level < 5 else 1.5
    hit = warriorA.maxAtk * c
    GetDamage() # Causes damage, defined just below these attacks
def atk2(*args):
    global hit, HT
    HT = warriorA.a2
    c = 1.3 if warriorA.level < 10 else 2
    if accurate(0.7 if warriorA.level < 10 else 0.6): hit = warriorA.maxAtk * c
    else: hit = 0 # Simply means Attack missed
    GetDamage()
def atk3(*args):
    global hit, HT
    HT = warriorA.a3
    c = 2 if warriorA.level < 10 else 2.5
    if accurate(0.35 if warriorA.level < 10 else 0.2): hit = warriorA.maxAtk * c
    else: hit = 0
    GetDamage()
...
def winScreen():
    ...
    canvas73.create_text(200,150,text=f"\n{warriorA} wins!",font=('showcard gothic',30),fill="yellow")
    warriorA.move_tank()
