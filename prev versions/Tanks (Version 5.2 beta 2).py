...
class Warrior:
    ...
    def move_tank(self):
        canvas73.move(self.base,1,0)
        canvas73.move(self.top,1,0)
        canvas73.move(self.turret,1,0)
...
def winScreen(): # incomplete function! need someone to create the ending 'CONGRATULATIONS' screen
    ...
    canvas73=Canvas(mainCanvas,height=250,width=400,bg="#000") 
    canvas73.pack()
    winText="CONGRATULATIONS!!! \n             "+ str(warriorA.name) + " WON!!!"
    canvas73.create_text(200,50,text=winText,font=(None,30),fill="yellow")
    warriorA.tank()
    warriorA.move_tank()
